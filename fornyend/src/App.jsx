...
export default function App() {
  ...
  const [products, setProducts] = useState([]); // Aapki main state
  ...
  const handleAddProduct = async (formData) => {
    try {
      ...
      // UI Update: Purani products + Nayi product jo backend se aayi
      setProducts((prev) => [...prev, response.data]);
      
      alert("Product added successfully!");
    } catch (error) {
      // Agar server band hai, toh error yahan ayega
      console.error("Backend Error:", error);
      alert("Connection failed! Check if server is running (node server.js).");
    }
  };
